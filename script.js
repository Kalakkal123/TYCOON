...
import { initFirstPerson, enableFirstPerson, disableFirstPerson, resizeFirstPersonCamera, updateFirstPerson } from "./firstperson.js";
...
(() => {
  ...
  class Visual3D {
    ...
    _initLights() {
      ...
      const hemi = new THREE.HemisphereLight(0xdbeafe, 0x475569, 0.5);
      this.scene.add(hemi);

      const dir = new THREE.DirectionalLight(0xfff5b6, 1.2);
      dir.position.set(this.worldWidth / 2 + 300, 800, this.worldHeight / 2 + 400);
      // Stronger sun-like directional light setup with shadow map coverage
      dir.target.position.set(this.worldWidth / 2, 0, this.worldHeight / 2);
      dir.castShadow = true;
    ...
    _initShip() {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        placeholder.position.set(this.worldWidth / 2 + 60 + i * 55, 8, this.worldHeight / 2 + 18); // Keep bottom flat on ground
    ...
    async _loadModels() {
      ...
      const safeLoad = async (key, path) => {
        try {
          ...
          console.error("Failed to load model:", key, path, err);
    ...
    syncCargo(ship, shipObj) {
      ...
      let woodCount = ship.loadWood + ship.loadGoodsWood;
    ...
    syncTruckCargo(game) {
      ...
      if (game.truck.state === "TO_FACTORY" || game.truck.state === "PROCESSING") {
          for (let i=0; i<game.truck.boxes; i++) {
             let m;
             if (this.models.box) {
                 m = this._buildInstance(this.models.box, 10, 0x8b5a2b, false);
             } else {
                 const geom = new THREE.BoxGeometry(8,8,8);
                 geom.translate(0, 4, 0); // Origin at bottom
                 m = new THREE.Mesh(geom, new THREE.MeshStandardMaterial({color:0x8b5a2b}));
                 m.castShadow = true;
             }
             m.position.set(0, i*9, 0);
          ...
          if (game.truck.load > 0) {
              ...
              let m;
              if (model) {
                  m = this._buildInstance(model, 15, tint, false);
              } else {
                  const geom = new THREE.BoxGeometry(12,12,12);
                  geom.translate(0, 6, 0);
                  m = new THREE.Mesh(geom, new THREE.MeshStandardMaterial({color: tint}));
                  m.castShadow = true;
              }
              this.truckCargo.add(m);
          }
    ...
    sync(game, alpha) {
      ...
      while(this.pickupBoxObjects.length < game.pickupBoxes) {
          let m;
          if (this.models.box) {
              m = this._buildInstance(this.models.box, 12, 0x8b5a2b, false);
          } else {
      ...
      for(let i=0; i<this.pickupBoxObjects.length; i++) {
          this.pickupBoxObjects[i].position.set(game.port.x - 40, i * 11, game.port.y - 20);
      }
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (i < game.ships.length) {
          ...
          const shipHeight = ship.state === "Traveling" || ship.state === "Returning" ? 0 : (ship.state === "Departing" ? Math.sin(nowMs() / 150) * 1.5 : 0);
      ...
      while (this.boatObjects.length < game.deliveries.length) {
        ...
        let m;
        if (this.models.boat) {
            m = this._buildInstance(this.models.boat, 40, null, true);
        } else {
            const geom = new THREE.BoxGeometry(32, 16, 48);
            geom.translate(0, 8, 0); // Origin at bottom
            m = new THREE.Mesh(geom, new THREE.MeshStandardMaterial({color:0x8b5a2b}));
            m.castShadow = true;
            m.receiveShadow = true;
        }
      ...
      for (let i = 0; i < this.boatObjects.length; i++) {
        ...
        if (i < game.deliveries.length) {
          ...
          obj.position.set(pos.x, Math.sin(d.t * Math.PI * 4) * 1.5, pos.y);
  ...
  class Game {
    constructor(stageEl, ui) {
      ...
      if (this._savedBotJobs) {
         for (let i = 0; i < this.bots.length; i++) {
            if (this._savedBotJobs[i]) this.bots[i].job = this._savedBotJobs[i];
         }
         delete this._savedBotJobs;
      }
      ...
      if (this.mineLevel > 0 && this.mineAllocationTarget > 0) {
         let assigned = 0;
         for(let i=0; i<this.bots.length; i++) {
             if (this.bots[i].job === "MINE" && assigned < this.mineAllocationTarget) {
    ...
    _initWorld() {
      ...
      const place = (type, count) => {
        for (let i = 0; i < count; i++) {
          let nx, ny;
          do {
            nx = lerp(margin, this.width - margin, this.rng());
            ny = lerp(margin, this.height - margin, this.rng());
          } while (dist({x: nx, y: ny}, {x: 80, y: 80}) < 150); // protect factory
          this.nodes.push(new ResourceNode(this._nodeId++, type, nx, ny));
        }
      };
      place("wood", 10);
      place("stone", 8);
    ...
    _wireUI() {
      ...
      if (this.ui.upgradeMine) {
        this.ui.upgradeMine.addEventListener("click", () => {
          if (this.mineLevel === 1) {
      ...
      this.visual.renderer.domElement.addEventListener("click", (e) => {
        ...
        if (dist({x: hit.x, y: hit.y}, {x: 80, y: 80}) < 150) return; // protect factory area from all clicks
      ...
      const chipShipWrapper = document.getElementById("chipShipWrapper");
      const shipDropdown = document.getElementById("shipDropdown");
      if (chipShipWrapper && shipDropdown) {
        chipShipWrapper.addEventListener("click", (e) => {
          if (e.target.closest("#shipDropdown")) return;
          shipDropdown.style.display = shipDropdown.style.display === "none" ? "block" : "none";
        });
      }

      const chipMoneyWrapper = document.getElementById("chipMoneyWrapper");
      const moneyDropdown = document.getElementById("moneyDropdown");
      if (chipMoneyWrapper && moneyDropdown) {
        chipMoneyWrapper.addEventListener("click", (e) => {
          if (e.target.closest("#moneyDropdown")) return;
          moneyDropdown.style.display = moneyDropdown.style.display === "none" ? "block" : "none";
        });
      }

      document.addEventListener("click", (e) => {
        if (chipShipWrapper && shipDropdown && !chipShipWrapper.contains(e.target)) {
          shipDropdown.style.display = "none";
        }
        if (chipMoneyWrapper && moneyDropdown && !chipMoneyWrapper.contains(e.target)) {
      ...
      try {
        ...
        if (version === 4 || version === 5 || version === 6) {
          ...
          if (Array.isArray(data.botJobs)) {
             this._savedBotJobs = data.botJobs;
          }
        }
        ...
        if (Array.isArray(data.ships)) {
          ...
          try {
            ...
            ship.timer = clamp(Number(s.timer ?? 0) || 0, 0, 999999);
            ship.halfTripSeconds = 30;
            ship.timer = Math.min(ship.timer, 30);
            this.ships = [ship];
            this._syncShips();
          } catch { 
            // ignore broken saves
    ...
    _pickTargetNode(bot) {
      ...
      for (const n of this.nodes) {
        ...
        if ((!bot.job || bot.job === "BASIC") && n.type !== desired) continue;
        const d = dist(bot, n);
        if (d < bestD) {
          bestD = d;
          best = n;
    ...
    update(dt) {
      ...
      let basicBots = this.bots.filter(b => !b.job || b.job === "BASIC");
      for (let i = 0; i < basicBots.length; i++) {
          basicBots[i].preferred = i % 2 === 0 ? "wood" : "stone";
      }
    ...
    syncUI(dt) {
      ...
      let loadWood = 0, loadStone = 0, loadGoodsWood = 0, loadGoodsStone = 0;
      ...
      const totalLoad = loadWood + loadStone + loadGoodsWood + loadGoodsStone;
      ...
      const shipDropdownState = document.getElementById("shipDropdownState");
      const shipDropdownTimer = document.getElementById("shipDropdownTimer");
      const shipDropdownLoad = document.getElementById("shipDropdownLoad");
      const shipDropdownValue = document.getElementById("shipDropdownValue");

      if (shipDropdownState) shipDropdownState.textContent = `${this.ships.length} Ships Active`;
      const value = loadWood * PRICES.wood + loadStone * PRICES.stone + loadGoodsWood * PRICES.goodsWood + loadGoodsStone * PRICES.goodsStone;
      if (shipDropdownLoad) shipDropdownLoad.textContent = `Total Load: ${totalLoad} / ${totalCap} (W:${loadWood}, S:${loadStone}, pW:${loadGoodsWood}, pS:${loadGoodsStone})`;
      if (shipDropdownValue) shipDropdownValue.textContent = `Value: $${value.toLocaleString()}`;
      if (shipDropdownTimer) shipDropdownTimer.textContent = minTime === Infinity ? "--:--" : formatTimer(minTime);
      ...
      const mineUnlockWrapper = document.getElementById("mineUnlockWrapper");
      if (mineUnlockWrapper) mineUnlockWrapper.style.display = this.mineLevel === 0 ? "block" : "none";

      const minePanel = document.getElementById("minePanel");
      if (minePanel) minePanel.style.display = this.mineLevel > 0 ? "block" : "none";

      if (upgradeMine) {
          if (this.mineLevel >= 2) {
              upgradeMine.style.display = "none";
          } else {
              upgradeMine.style.display = "";
              upgradeMine.disabled = this.gems < 10 || this.gold < 200;
          }
      }
  ...
  function main() {
    ...
    const btnChangeView = document.getElementById("btnChangeView");
    if (btnChangeView) {
      let fpEnabled = false;
      btnChangeView.addEventListener("click", () => {
        fpEnabled = !fpEnabled;
        if (fpEnabled) enableFirstPerson();
    ...
    try {
      const ui = {
        ...
        upgradeMine: document.getElementById("upgradeMine"),
