...
export const SHIP_CYCLE_SECONDS_MIN = 60;
export const SHIP_CYCLE_SECONDS_MAX = 60;

export const GEM_CHANCE_ON_RETURN = 0.07;

export const MINE_UNLOCK_COST_GEMS = 5;
export const MINE_MIN_BOTS = 5;
export const MINE_MAX_BOTS = 15;
...
export class Ship {
  ...
  update(dt, game) {
    ...
    if (!game.shipFilterWood && !game.shipFilterStone) {
    ...
    const targetW = (game.shipFilterWood && game.shipFilterStone) ? Math.ceil(this.capacity / 2) : (game.shipFilterWood ? this.capacity : 0);
    const targetS = (game.shipFilterWood && game.shipFilterStone) ? Math.floor(this.capacity / 2) : (game.shipFilterStone ? this.capacity : 0);

    const currentW = this.loadWood + this.loadGoodsWood;
    const currentS = this.loadStone + this.loadGoodsStone;
    ...
    const takeW = currentW < targetW && availableW > 0;
    const takeS = currentS < targetS && availableS > 0;
    ...
    if (takeW && takeS) {
      if (currentW <= currentS) loadOne("wood");
      else loadOne("stone");
    } else if (takeW) {
      loadOne("wood");
    } else if (takeS) {
